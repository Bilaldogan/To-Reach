//
//  HttpAdress.swift
//  App To Rich
//
//  Created by Baran on 7.10.2017.
//  Copyright © 2017 bilal. All rights reserved.
//

import Foundation

struct HttpAdress {
    
    static let staticLink : String = "http://www.apptorich.com/AppToRichService.asmx?op="
    
    static let registerServiceLink : String = "UserAddService"
    static let loginServiceLink : String = "LoginService"
    static let mainServiceLink : String = "AppListService"
    static let userProfileServicelink : String = "UserProfilService"
    static let clickAdwerdLink : String = "AddAppService"
    static let forgotPassLink : String = "ForgotPassService"
    static let transferServiceLink : String = "UserMoneyService"
}
